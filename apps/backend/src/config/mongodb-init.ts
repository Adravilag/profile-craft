...
export const initializeMongoDB = async (): Promise<void> => {
  try {
    ...
    if (!existingAdmin) {
      ...
      const defaultSkills = [
        { user_id: defaultAdmin._id, name: 'JavaScript', category: 'Frontend', level: 90, order_index: 1 },
        { user_id: defaultAdmin._id, name: 'TypeScript', category: 'Frontend', level: 85, order_index: 2 },
        { user_id: defaultAdmin._id, name: 'React', category: 'Frontend', level: 90, order_index: 3 },
        { user_id: defaultAdmin._id, name: 'Node.js', category: 'Backend', level: 85, order_index: 4 },
        { user_id: defaultAdmin._id, name: 'Express.js', category: 'Backend', level: 80, order_index: 5 },
        { user_id: defaultAdmin._id, name: 'MongoDB', category: 'Database', level: 75, order_index: 6 },
      ...
      const defaultProject = new Project({
        ...
        description: 'API backend completa para portfolio personal con autenticación JWT, gestión de proyectos y MongoDB.',
        image_url: null,
        github_url: 'https://github.com/example/profilecraft-backend',
        live_url: 'https://profilecraft-api.onrender.com',
        article_url: null,
        article_content: 'Este es el backend completo de ProfileCraft, desarrollado con Node.js, Express, TypeScript y MongoDB. Incluye autenticación JWT, gestión de usuarios, proyectos, habilidades y experiencias laborales.',
        video_demo_url: null,
        status: 'Completado',
        order_index: 1,
        type: 'proyecto',
