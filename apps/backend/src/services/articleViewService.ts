// src/services/articleViewService.ts
...
/**
 * Servicio para manejar las visualizaciones de artículos
 */
export class ArticleViewService {
  ...
  static async getUniqueViews(projectId: string): Promise<number> {
    try {
      const uniqueIPs = await ArticleView.distinct('ip_address', {
        project_id: projectId
      });
  ...
  static async getViewStats(projectId: string) {
    try {
      const totalViews = await ArticleView.countDocuments({ project_id: projectId });
      const uniqueViews = await this.getUniqueViews(projectId);
      
      // Vistas en los últimos 7 días
      const weekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      const recentViews = await ArticleView.countDocuments({
        project_id: projectId,
        viewed_at: { $gte: weekAgo }
      });

      // Vistas por día en la última semana
      const dailyViews = await ArticleView.aggregate([
        {
          $match: {
            project_id: projectId,
            viewed_at: { $gte: weekAgo }
  ...
  static async cleanOldViews(): Promise<number> {
    try {
      ...
      const result = await ArticleView.deleteMany({
