...
export const cloudinaryService = {
   ...
   */
  uploadProfileImage: async (buffer: Buffer, originalName: string): Promise<CloudinaryUploadResult> => {
    try {
      // Verificar si Cloudinary está configurado
      if (!isCloudinaryConfigured()) {
        throw new Error('Cloudinary no está configurado. Por favor, configura las variables de entorno CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY y CLOUDINARY_API_SECRET.');
      }
   ...
   */
  uploadProjectImage: async (buffer: Buffer, originalName: string): Promise<CloudinaryUploadResult> => {
    try {
      // Verificar si Cloudinary está configurado
      if (!isCloudinaryConfigured()) {
        throw new Error('Cloudinary no está configurado. Por favor, configura las variables de entorno CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY y CLOUDINARY_API_SECRET.');
