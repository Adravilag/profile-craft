...
const ContactTooltips: React.FC<ContactTooltipsProps> = ({ 
  contacts, 
  compact = false 
}) => {
  ...
  const getActionForContact = (contact: ContactInfo) => {
    switch (contact.type) {
      ...
      case 'location':
        return () => window.open(`https://maps.google.com?q=${encodeURIComponent(contact.value)}`, '_blank');
      default:
        return undefined;
    }
  };
            ...
            <div className="contact-tooltip">
              <div className="tooltip-content">
                ...
                <div className="tooltip-actions">
                  <button 
                    className="tooltip-action primary"
                    onClick={getActionForContact(contact)}
                  >
                    {contact.type === 'email' ? 'Enviar' : 
                     contact.type === 'location' ? 'Ver mapa' : 'Abrir'}
                  </button>
                  <button 
