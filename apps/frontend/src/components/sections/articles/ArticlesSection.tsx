...
const ArticlesSection: React.FC<ArticlesSectionProps> = ({
  ...
  // Efecto para ajustar el filtro por defecto basado en el contenido disponible
  useEffect(() => {
  ...
  // Funciones para obtener título e icono dinámicos
  ...
  const getDynamicSubtitle = () => {
    if (filteredTotalArticles > articlesPerPage) {
      const contentType = selectedFilter === 'all' ? 'elementos' : 
                         selectedFilter === 'projects' ? 'proyectos' : 'artículos';
    ...
    switch (selectedFilter) {
      ...
      case 'articles':
        return 'Una selección de mis artículos más interesantes y útiles';
      default:
