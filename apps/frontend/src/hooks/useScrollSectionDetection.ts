...
export const useScrollSectionDetection = (config: SectionDetectionConfig = {}) => {
  ...
  const detectVisibleSection = useCallback((): string | null => {
    ...
    // Si estamos en la parte superior (header visible), mostrar "about"
    ...
    for (const sectionId of sections) {
      ...
      const visibilityRatio = intersectionHeight / Math.min(rect.height, viewportHeight - navHeight);
   ...
   */
  const disableDetection = useCallback((duration: number = 1500) => {
