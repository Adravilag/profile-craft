...
export const useFilterFAB = ({ debug = false }: UseFilterFABProps = {}): FilterFABState => {
  ...
  const [shouldShowByNavigation, setShouldShowByNavigation] = useState(isOnSkillsURL || isInSkillsSection);
  ...
  useEffect(() => {
    ...
    if (newShouldShow !== shouldShowByNavigation) {
      if (debug) console.log(`[SKILLS_FILTER] Cambio de sección detectado: ${newShouldShow ? 'entrando a skills' : 'saliendo de skills'}`);
  ...
  useEffect(() => {
      ...
      observerRef.current = new IntersectionObserver(
        (entries) => {
          ...
          if (debug) {
            ...
            console.log(`[SKILLS_FILTER_DEBUG] Intersection ratio: ${entry.intersectionRatio.toFixed(3)}`);
            console.log(`[SKILLS_FILTER_DEBUG] Sección activa: ${document.body.getAttribute('data-active-section')}`);
    ...
    if (skillsSectionElem) {
      ...
      const skillsSectionBySelector = document.querySelector('section[id="skills"], .skills-section, section.section-cv');
  ...
  const isActiveByAttribute = activeSection === 'skills';
  
  const isVisible = shouldShowByNavigation && shouldShow && isActiveByAttribute;
  ...
  useEffect(() => {
    if (debug) {
      ...
      console.log(`[SKILLS_FILTER] Sección activa: ${activeSection}`);
    }
    
    // Forzar la eliminación de la clase si no estamos en la sección de skills
    if (!isActiveByAttribute || !shouldShowByNavigation) {
      document.body.classList.remove('in-skills-section');
    }
  }, [isVisible, activeSection, debug, shouldShowByNavigation, isActiveByAttribute]);
