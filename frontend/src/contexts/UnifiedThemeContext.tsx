...
export const UnifiedThemeProvider: React.FC<UnifiedThemeProviderProps> = ({ children }) => {
  ...
  const [preferences, setPreferences] = useState<ThemePreferences>(() => {
    ...
    const oldArticlePrefs = localStorage.getItem('article-theme-preferences');
    ...
    if (oldArticlePrefs) {
      try {
        const parsed = JSON.parse(oldArticlePrefs);        migratedPrefs = {
          ...migratedPrefs,
  ...
  useEffect(() => {    localStorage.setItem('unified-theme-preferences', JSON.stringify(preferences));
    
    // Mantener compatibilidad con sistemas legacy
    localStorage.setItem('cv-theme', preferences.globalTheme);
    localStorage.setItem('article-theme-preferences', JSON.stringify({
  ...
  useEffect(() => {
    ...
    body.classList.remove('article-theme-light', 'article-theme-dark', 'article-theme-sepia');
    body.classList.add(`article-theme-${currentGlobalTheme}`);
    ...
    if (metaThemeColor) {
      metaThemeColor.setAttribute(
        'content', 
        currentGlobalTheme === 'dark' ? '#101418' : '#fdfcff'
      );
    }
