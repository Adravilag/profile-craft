...
export type ArticleTheme = 'light' | 'dark' | 'sepia' | 'auto';
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
  const [preferences, setPreferences] = useState<ThemePreferences>(() => {
    const saved = localStorage.getItem('article-theme-preferences');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('article-theme-preferences', JSON.stringify(preferences));
  }, [preferences]);
  ...
  useEffect(() => {
    ...
    // Aplicar tema actual
    body.classList.add(`article-theme-${currentTheme}`);
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [preferences.autoNightMode, preferences.theme, preferences.nightModeStart, preferences.nightModeEnd, currentTheme]);
