...
const LexicalEditorNew: React.FC<SimpleLexicalEditorProps> = ({
  ...
  const getHtmlSuggestions = (currentText: string, cursorPos: number): string[] => {
    ...
    const commonTags = [
      ...
      'figure', 'figcaption', 'blockquote', 'code', 'pre', 'strong', 'em',
  ...
  const convertMarkdownToHtml = (markdown: string): string => {
    ...
    if (processedLines.some(line => line.includes('<tbody>') && !processedLines.some(l => l.includes('</tbody>')))) {
    ...
    markdown = markdown.replace(/<table[^>]*>(.*?)<\/table>/gis, (_, content) => {
      ...
      if (theadMatch) {
        ...
        headerRows.forEach((row: string) => {
          ...
          const cellTexts = cells.map((cell: string) => cell.replace(/<th[^>]*>(.*?)<\/th>/gi, '$1').trim());
      ...
      if (tbodyMatch) {
        ...
        dataRows.forEach((row: string) => {
          ...
          const cellTexts = cells.map((cell: string) => cell.replace(/<td[^>]*>(.*?)<\/td>/gi, '$1').trim());
          tableMarkdown += '| ' + cellTexts.join(' | ') + ' |\n';
        });
      }
  ...
  const handleModeConversion = (newMode: ViewMode) => {
    ...
    if (newMode === 'markdown' && viewMode !== 'markdown') {
      ...
      console.log('Converting to Markdown. Current content:', currentContent.substring(0, 100) + '...');
      ...
      console.log('Converting from Markdown to HTML. Current content:', currentContent.substring(0, 100) + '...');
  ...
  const renderPreview = () => {
    ...
    return (
      <div className={advancedStyles.contentPreview}>
        <div
          className={`${advancedStyles.previewContent} article-content`}
  ...
  const handleMediaSelection = (url: string) => {
    if (textAreaRef.current) {
      ...
      if (isImage) {
        const altText =
          prompt("Texto alternativo para la imagen:", "Imagen") || "Imagen";
...
<video controls style="max-width: 100%; height: auto; display: block; margin: 20px 0;">
  ...
  Tu navegador no soporta la etiqueta de video.
        ...
        mediaHtml = `<a href="${url}" target="_blank" rel="noopener noreferrer" class="article-link" style="color: var(--md-sys-color-primary);">${url
          .split("/")
          .pop()}</a>`;
      }
                  ...
                  <div className={styles.htmlActionGroup}>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (url) {
                          wrapWithTag('a', `href="${url}" target="_blank" rel="noopener noreferrer"`);
                        }
                      }}
                      ...
                      if (htmlContent.trim()) {
                        try {
                          const formatHTML = (html: string): string => {
                            ...
                            const inlineElements = ['span', 'strong', 'em', 'b', 'i', 'a', 'code', 'small', 'mark', 'sub', 'sup'];
                            
                            // Elementos que siempre deben estar en nueva línea
                            const blockElements = ['div', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'li', 'section', 'article', 'header', 'footer', 'nav', 'main', 'aside', 'blockquote', 'pre', 'table', 'thead', 'tbody', 'tr', 'td', 'th'];
                            
                            // Elementos auto-cerrados
                            const selfClosingElements = ['img', 'br', 'hr', 'input', 'meta', 'link', 'area', 'base', 'col', 'embed', 'source', 'track', 'wbr'];
                            
                            // Aplicar formateo básico
                            blockElements.forEach(tag => {
                              cleanHtml = cleanHtml.replace(new RegExp(`<${tag}([^>]*)>`, 'gi'), `\n<${tag}$1>\n`);
                              cleanHtml = cleanHtml.replace(new RegExp(`</${tag}>`, 'gi'), `\n</${tag}>\n`);
                            });
                            
                            selfClosingElements.forEach(tag => {
                              cleanHtml = cleanHtml.replace(new RegExp(`<${tag}([^>]*)/?>`, 'gi'), `<${tag}$1/>\n`);
                            });
                          ...
                          alert("❌ Error al formatear el HTML. Verifica que el código sea válido.");
                        }
                      }
                    }}
                    ...
                    onClick={() => {
                      ...
                      if (htmlContent.trim()) {
                        try {
                          ...
                          alert(`❌ Error al validar HTML: ${error instanceof Error ? error.message : 'Error desconocido'}`);
                        }
                      } else {
                    ...
                    onClick={() => {
                      ...
                      if (htmlContent.trim()) {
                        try {
                          ...
                          alert(`❌ Error al sanear el HTML: ${error instanceof Error ? error.message : 'Error desconocido'}`);
                        }
                      }
                    }}
                    ...
                    onClick={() => {
                      ...
                      if (htmlContent.trim()) {
                        try {
                          ...
                          alert(`❌ Error al minificar HTML: ${error instanceof Error ? error.message : 'Error desconocido'}`);
                        }
                      }
                    }}
                    ...
                    onClick={() => {
                      ...
                      if (htmlContent.trim()) {
                        try {
                          ...
                          if (badHeaders) {
                            warnings.push(`⚠️ ${badHeaders.length} encabezado(s) sin espacio después de #`);
                          }
                          ...
                          alert(`❌ Error al validar Markdown: ${error instanceof Error ? error.message : 'Error desconocido'}`);
                        }
                      } else {
                        alert("ℹ️ No hay contenido para validar");
                      }
                      ...
                      if (htmlContent.trim()) {
                        try {
                          const generateTOC = (markdown: string): string => {
                            ...
                            if (headers.length === 0) {
                              throw new Error('No se encontraron encabezados para generar tabla de contenidos');
                            }
                          ...
                          alert(`❌ ${error instanceof Error ? error.message : 'Error al generar tabla de contenidos'}`);
                        }
                      }
                    }}
            ...
            <textarea
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "Enter") {
                  ...
                  const currentLine = textBeforeCursor.substring(
                    lastNewlineIndex + 1
                  );
                ...
                if (e.key === ">" && textAreaRef.current) {
                  ...
                  if (
                    ...
                    !["br", "img", "input", "hr", "meta", "link"].includes(
                      openTagMatch[1]
                    )
                  ) {
                    e.preventDefault();
                    ...
                    const newContent =
                      textBeforeCursor + ">" + htmlContent.substring(cursorPos);
                ...
                if (viewMode === 'html' && (e.ctrlKey || e.metaKey)) {
                  switch (e.key) {
                    ...
                    case '/':
                      ...
                      const lineEnd = nextNewlineIndex === -1 ? htmlContent.length : nextNewlineIndex;
                ...
                if (viewMode === 'markdown' && (e.ctrlKey || e.metaKey)) {
                  switch (e.key) {
                    case 'b':
                      ...
                      const selectedTextB = htmlContent.substring(startB, endB) || "texto en negrita";
                      const newContentB = 
                    ...
                    case 'i':
                      ...
                      const selectedTextI = htmlContent.substring(startI, endI) || "texto en cursiva";
                      const newContentI = 
                    ...
                    case '6':
                      ...
                      const selectedTextH = htmlContent.substring(startH, endH) || `Encabezado ${level}`;
                      const newContentH = 
              ...
              style={{
                ...
                lineHeight: "1.5",
                tabSize: 2,
                backgroundColor: viewMode === 'html' ? 'var(--md-sys-color-surface-container)' : 
                                viewMode === 'markdown' ? 'var(--md-sys-color-surface-variant)' : 'white',
                color: viewMode === 'html' || viewMode === 'markdown' ? 'var(--md-sys-color-on-surface)' : 'inherit',
              }}
            />
          </div>
        )}
        ...
        {shouldShowMarkdownTools() && (
          <div className={styles.markdownStatusBar}>
            <div className={styles.markdownStats}>
              <span>Líneas: {htmlContent.split('\n').length}</span>
              <span>Palabras: {htmlContent.split(/\s+/).filter(word => word.length > 0).length}</span>
              <span>Caracteres: {htmlContent.length}</span>
