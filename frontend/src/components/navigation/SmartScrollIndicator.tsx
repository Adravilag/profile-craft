  ...
  useEffect(() => {    const handleScroll = () => {
      ...
      if (currentSectionElement) {
        ...
        if (scrollY + navHeight >= sectionTop && scrollY < sectionTop + sectionHeight) {
          ...
          sectionProgress = Math.min(sectionScrolled / (sectionHeight - viewportHeight + navHeight), 1);
        ...
        <div className="mini-section-nav">
          {sections.map((section) => (
            <div
              ...
              title={section.label}
              style={{
                backgroundColor: section.id === currentSection ? '#6366f1' : 'rgba(255,255,255,0.3)'
              }}
            />
          ))}
        </div>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="scroll-debug-info">
          <small>
            ...
            Progress: {Math.round(scrollState.sectionProgress * 100)}% |
